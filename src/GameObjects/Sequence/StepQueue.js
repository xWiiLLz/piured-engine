...
class StepQueue extends GameObject {
    ...
    getFirstStepWithinMargin(currentAudioTime, kind, padId) {
        for (var i = 0; i < this.stepQueue.length; i++) {
            ...
            const difference = timeStamp - currentAudioTime;
            const differenceAbs = Math.abs(difference);

            // console.log(difference) ;

            if (differenceAbs < this.accuracyMargin) {
    ...
    stepPressed(kind, padId) {
        ...
        let [stepInfo, step, hitIndex, difference] =
            this.getFirstStepWithinMargin(currentAudioTime, kind, padId);
