...
export class Explosion extends GameObject {
    ...
    update(delta: number) {
        if (this._animationPosition <= 4) {
            ...
            const timeStamp = this._lastStepTimeStamp + delta;
            this._mesh.material.opacity = 1.0;
            const movement = timeStamp / (this._explosionAnimationRate * 0.016);
            ...
            if (movement > 1) {
                ...
                this._lastStepTimeStamp =
                    timeStamp % (this._explosionAnimationRate * 0.016);
            } else {
